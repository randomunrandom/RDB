procedure Main()
    ...
    enddo
    ? '2'
    index on descend(name) to 'index_name_reverse'
    go top
    do while !eof()
        ? name
        if sex
    ...
    enddo
    ? '3'
    index on if(sex, '0', '1')+name to 'index_sex'
    go top
    do while !eof()
        ? name
        if sex
    ...
    enddo
    ? '4'
    index on if(sex, '0'+name, '1'+descend(name)) to 'index_sex'
    go top
    do while !eof()
        ? name
        if sex
    ...
    enddo
    ? '5'
    index on month(bday) to 'index_sex'
    go top
    do while !eof()
        ? name
        if sex
    ...
    enddo
    ? '6'
    index on str(month(bday), 8, 0)+str(pay, 20, 2) to 'index_sex'
    go top
    do while !eof()
        ? name
        if sex
...
procedure create_db()
    ...
    dbcreate('lab02_tabl01', struct)

    use lab02_tabl01 new
    append blank
