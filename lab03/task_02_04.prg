procedure Main()
    ...
    index on lab03_groups -> spec + group_abr + name to 'studdb_name'
    ?
    ? '4'
    ?
    go top
    tmp_spec = lab03_groups -> spec
    tmp_gr_name = group_abr
    ? tmp_spec, tmp_gr_name
    do while !eof()
        if lab03_groups -> spec # tmp_spec .and. group_abr # tmp_gr_name
            ? lab03_groups -> spec, group_abr
        endif
        tmp = lab03_groups -> spec
        tmp_gr_name = group_abr
        ? '    ', name
        skip
    enddo
